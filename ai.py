...
class AI(object):
    def __init__(self, n_inputs, n_actions, hidden_neurons, learning_rate):
        self.inputs = tf.placeholder(shape=(n_inputs,), dtype=tf.float32)
        self.inputs_reshaped = tf.reshape(self.inputs, (1,-1))
        self.W1 = tf.Variable(tf.random_uniform((n_inputs, hidden_neurons), 0, 1))
        self.x1 = tf.matmul(self.inputs_reshaped, self.W1)
        self.W2 = tf.Variable(tf.random_uniform((hidden_neurons, n_actions), 0, 1))
        self.Q = tf.reshape(tf.matmul(self.x1, self.W2), (-1,))

        self.W1_ = tf.Variable(self.W1)
        self.x1_ = tf.matmul(self.inputs_reshaped, self.W1_)
